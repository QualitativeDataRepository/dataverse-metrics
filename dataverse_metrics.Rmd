...
dataverse_host <- "http://demo.dataverse.org"
#dataverse_host <- "http://dataverse.harvard.edu"
...
if (file.exists("deposits.rds")) {
  ...
  deposits_tb <- tibble(deposit=deposits$data$items) %>% unnest_wider(deposit)

  # Do we need to paginate?
  while (deposits$data$start + deposits$data$count_in_response < deposits$data$total_count) {
    Sys.sleep(3) # Be NICE to the API! This can take a while if there's lots of projects
    start <- deposits$data$start + deposits$data$count_in_response
    message(paste("at", start, "of", deposits$data$total_count))
    deposits <- content(GET(paste0(dataverse_host,
                                   "/api/search/?q=*&type=dataset&per_page=1000",
                                   "&start=", start),
                            config=add_headers("X-Dataverse-Key"=dataverse_key)))
    deposits_tb <- tibble(deposit=deposits$data$items) %>% unnest_wider(deposit) %>% 
      bind_rows(deposits_tb)
  }
  
  deposits <- deposits_tb
  rm(deposits_tb)
  ...
  projects <- content(GET(paste0(dataverse, "/uniquedownloads")))
...
if (any(deposits$versionState=="Unpublished")) {
  subj_tbl_a <- deposits %>% filter(versionState=="Unpublished") %>% select(c("subjects")) %>%
    unnest_longer(subjects) %>% group_by(subjects) %>%
    summarize(unpublished=n()) %>% full_join(deposits_subj_published) %>%
    knitr::kable(caption="Deposits by subject",
                 col.names=c("Subject", "Draft", "Published"))
  
  if (any(subset(deposits, as.Date(createdAt) < period_end &
                 as.Date(createdAt) > period_begin)$versionState=="Unpublished")) {
    subj_tbl_b <- deposits %>% filter(versionState == "Unpublished" & 
                          as.Date(createdAt) < period_end &
                          as.Date(createdAt) > period_begin) %>%
      select(c("subjects")) %>% unnest_longer(subjects) %>% group_by(subjects) %>%
      summarize(unpublished=n()) %>% full_join(deposits_subj_period_published) %>%
      knitr::kable(caption=paste("Deposits by subject created between", 
                                 period_begin,
                                 "and", period_end),
                   col.names=c("Subject", "Draft", "Published")) 
  }  else {
    subj_tbl_b <- deposits_subj_period_published %>% 
  ...
  subj_tbl_a <- deposits_subj_published %>% 
        knitr::kable(caption="Deposits by subject",
                 col.names=c("Subject", "Published"))
}
